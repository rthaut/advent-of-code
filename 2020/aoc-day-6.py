...
def get_input_as_grouped_lists(input):
	groups = []
	for group in input.split("\n\n"):
		groups.append(group.split("\n"))
	return groups

def get_unique_answers_for_group(group):
	answers = set(group[0])
	for person in group[1:]:
		answers.update(set(person))
	return answers

def get_same_answers_for_group(group):
	answers = set(group[0])
	for person in group[1:]:
		answers.intersection_update(set(person))
	return answers

def get_sum_of_unique_answers_for_groups(groups):
	count = 0
	for group in groups:
		count += len(get_unique_answers_for_group(group))
	return count

def get_sum_of_same_answers_for_groups(groups):
	count = 0
	for group in groups:
		count += len(get_same_answers_for_group(group))
	return count

groups = get_input_as_grouped_lists(input)

sum_unique = get_sum_of_unique_answers_for_groups(groups)
print("sum of unique questions per group: " + str(sum_unique))

sum_same = get_sum_of_same_answers_for_groups(groups)
print("sum of same questions per group: " + str(sum_same))
