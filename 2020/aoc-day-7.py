...
for rule in input_rules.split("\n"):
	...
	color = rule_parts[0].replace(" bags", "")
	rules[color] = {}
	contents = rule_parts[1].split(",")
	if contents[0] == "no other bags.":
		continue
	for content in contents:
		matches = re.search(r"^(\d+) (.*) bags?\.?$", content.strip())
		if matches:
			rules[color][matches.group(2)] = int(matches.group(1))

def get_bags_containing_color(color):
	bags = []
	for rule in rules:
...
def get_bag_count_for_color(color):
	if len(rules[color].keys()) == 0:
		# this bag does not contain any bags
		return 0
	count = 0
	for bag in rules[color]:
		# the number of bags for this color, plus the number of bags in each of those bags (recursive)
		count += rules[color][bag] + (rules[color][bag] * get_bag_count_for_color(bag))
	return count

# starting set of bags that directly hold shiny gold
bags_holding_gold = set(get_bags_containing_color("shiny gold"))

# this set will be used in the loop to hold bags that hold other bags that hold gold
new_bags = set(bags_holding_gold)
loops = 0 # just for debugging
while len(list(new_bags)) > 0 and loops < 100: # loop limit is just to prevent infinite loop during development
	loops += 1
	for bag in list(new_bags):
		new_bags.update(get_bags_containing_color(bag))
	# we only want the set of bags that are actually new for this iteration
	new_bags = set(new_bags - bags_holding_gold)
	# print("New bags for loop " + str(loops) + ": " + ",".join(new_bags))
	bags_holding_gold.update(new_bags)

# print("loops: " + str(loops))

print("bags that can eventually hold a shiny gold bag: " + str(len(bags_holding_gold)))
print("maximum possible bags in a shiny gold bag: " + str(get_bag_count_for_color("shiny gold")))
