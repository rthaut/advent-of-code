...
def validateField(key, value):
	if key == "byr":
		...
		match = re.search(r"^(\d+)(\w{2})$", value)
		if match != None:
			if match.group(2) == "cm":
				# If cm, the number must be at least 150 and at most 193
				return 150 <= int(match.group(1)) and int(match.group(1)) <= 193
			elif match.group(2) == "in":
				# If in, the number must be at least 59 and at most 76
				return 59 <= int(match.group(1)) and int(match.group(1)) <= 76
			else:
				return False;
	elif key == "hcl":
...
def validatePassport(passport):
	...
	for field in fields:
		...
		fieldsDict[elements[0]] = validateField(elements[0], elements[1])
		# print("Passport field '{}' ('{}') validation result: {}".format(elements[0], elements[1], str(fieldsDict[elements[0]])))
	if (all(field in list(fieldsDict.keys()) for field in requiredFields)):
		# print("Passport contains all required fields")
		if (all(list(fieldsDict.values()))):
			# print("All passport fields are valid")
			return True
	return False
